...
locals {
  ...
  minio                = "%{if var.https == true}https://%{else}http://%{endif}%{if var.service_type == "ClusterIP"}minio.${replace(var.external_url, "coder.", "")}%{else}${var.external_url}:30080%{endif}"
  ...
  krm                  = "%{if var.https == true}https://%{else}http://%{endif}%{if var.service_type == "ClusterIP"}krm.${replace(var.external_url, "coder.", "")}%{else}${var.external_url}:30220%{endif}"
}
