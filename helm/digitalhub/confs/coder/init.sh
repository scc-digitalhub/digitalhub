...
coder templates create -d /home/coder/{{ .name }} {{ .name }} --variable namespace={{ $.Release.Namespace }},service_type={{ $.Values.global.service.type }},node_port={{ .nodePort }}{{ if eq .name "dashboard" }},external_url={{ $.Values.global.externalHostAddress }}{{ end }} -y 
