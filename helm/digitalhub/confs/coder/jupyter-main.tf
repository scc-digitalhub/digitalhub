...
locals {
  jupyter_url = "%{if var.https == true}https://%{else}http://%{endif}%{if var.service_type == "ClusterIP"}jupyter--${data.coder_workspace.me.name}--jupyter--${data.coder_workspace.me.owner}.${var.external_url}%{else}${var.external_url}:${var.node_port}%{endif}"
}
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "jupyter"
          image             = "ghcr.io/scc-digitalhub/jupyter:1.4.0_0.0.3"
          image_pull_policy = "Always"
