...
{{/*
Coder ingress environment variables.
*/}}
...
- name: CODER_WILDCARD_ACCESS_URL
  value: {{ .Values.coder.ingress.wildcardHost | quote }}
- name: CODER_ACCESS_URL
  value: {{ printf "%s://%s" $proto .Values.coder.ingress.host | quote }}
{{- end }}
{{- end }}
...
{{- define "coder.defaultAccessURL" }}
  ...
  {{- if eq (include "coder.tlsEnabled" .) "true" -}}
    {{- $proto = "https" -}}
      {{- if eq .Values.coder.service.type "NodePort" -}}
        {{- $port = .Values.coder.service.httpsNodePort -}}
      {{- end -}}
  {{- else -}}
  {{- $proto = "http" -}}
    {{- if eq .Values.coder.service.type "NodePort" -}}
  ...
  {{- else -}}
    {{ $proto }}://coder.{{ .Release.Namespace }}.svc.cluster.local
  {{- end }}
{{- end }}
